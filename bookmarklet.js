(function(){
  ...
  if (document.querySelector('#script-notice-box')) {
    return;
  }

  function simulateHoverForEl(element) {
  ...
  function waitForElement(selector, callback, retries = 20, delay = 500) {
    const check = () => {
  ...
  function selectLatestMonth() {
    const select = document.querySelector('#Donem_Id');
    if (!select) {
      alert('Ay seçim kutusu bulunamadı.');
      return false;
    }

    const targetOption = Array.from(select.options).find(opt => opt.value !== 'null');
    if (!targetOption) {
      alert('Seçilebilir bir ay bulunamadı.');
      return false;
    }
  ...
  function simulateAllAndRunApp() {
    const parentLink = Array.from(document.querySelectorAll('li > a')).find(el =>
      el.textContent.includes('PERSONEL')
    );

    if (!parentLink) {
      alert('"PERSONEL" menüsü bulunamadı.');
      return;
    }

    simulateHoverForEl(parentLink);

    setTimeout(() => {
      const pdksLink = Array.from(document.querySelectorAll('a')).find(el =>
        el.textContent.includes('PDKS Giriş-Çıkış Bilgileri Kartı')
      );
      if (!pdksLink) {
        alert('"PDKS Giriş-Çıkış Bilgileri Kartı" menü öğesi bulunamadı.');
        return;
      }

      pdksLink.click();

      waitForElement('#Donem_Id', () => {
        const selected = selectLatestMonth();
        if (!selected) return;

        waitForGridRows('#grid_kesin_giris_cikis', () => {
          addNoticeBox();
          app();
        });
      });
